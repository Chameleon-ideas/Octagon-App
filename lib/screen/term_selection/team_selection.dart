            ...
            body: BlocConsumer(
              ...
              listener: (context,state){
                ...
                if(state is SaveTermListState){
                  ...
                  ///first team flag
                  storage.write('userDefaultTeam', selectedTeamLogo.isNotEmpty?selectedTeamLogo : state.saveSportListResponseModel.success!.sportInfo!.first.team!.first.strTeamLogo.toString());
                  storage.write('userDefaultTeamName', selectedTeam?.toJson());

                  if(widget.isUpdate){
                    Navigator.pushReplacement(
                        context, MaterialPageRoute(builder: (context) => TabScreen(selectedPage: 3,)));
                  }else{
                    Navigator.pushReplacement(
  ...
  buildNextButton() {
   return Container(
        ...
        child: FilledButtonWidget(isLoading: isLoading,/*widget.model, */widget.isUpdate? "Update":"Next", () {
          ...
          for (var element in teamListResponseModel
    ...
    widget: Container(
      ...
      child: ShapeMaker(
        ...
        widget: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Center(
              child: ImageViewWidget(image: image ?? "")
          ),
        ),
      ),
    ),
