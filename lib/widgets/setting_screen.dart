...
class _SettingScreenState extends State<SettingScreen> {
 ...
 // late PostBloc postBloc;
  ...
  void initState() {
    ...
    accountsMenuRows = [
      _menuRowWithFunction(
          Icons.person, "Edit Profile", (){
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>
                    EditProfileScreen(profileData: widget.profileData?.success?.user, update: (UserModel data){
                    },)));
      }),
      _menuRowWithFunction(
          Icons.password_outlined, "Reset Password", (){
    ...
    teams = [
      _menuRowWithFunction(Icons.person, "Teams", (){
        ...
        Navigator.push(context, MaterialPageRoute(
            builder: (context) => SportSelection( sportDataList: sportDataList, isUpdate: true,
            )));
      }),
    ];
    contentAndDisplayRows = [
      _menuRowWithFunction(Icons.notifications, "Notification",(){
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => TabScreen(selectedPage: 2,)));
      }),
      _menuRowWithFunction(
          Icons.near_me_rounded, "Share", (){
        Share.share('My Favourite app for sports https://octagonapp.com/app-download');///${Platform.isIOS ? "https://apps.apple.com/us/app/octagon-app/id1673110067":"https://play.google.com/store/apps/details?id=com.octagon.app"}
      }),
      _menuRowWithFunction(
          Icons.person_off, "Blocked Users", (){
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => BlockUserListScreen()));
      }),
    ];
    cacheAndCellularRows = [
      _menuRowWithFunction(Icons.delete_rounded, "Clear Cache", () {
        _handleForgetMeClick.call(context, title: 'This will clear local cache memory!.',
            onYes: (){
    ...
    aboutMenuRows = [
      _menuRowWithFunction(
          Icons.contact_support_rounded, "Contact Us", (){
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => WebViewScreen(screenName: "Contact Us", url: contactUsUrl,)));
      }),
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(children: [
                ...
                _menuSection(
                    menuList: _commonMenu(accountsMenuRows),
                    titleString: "Account"),
                _menuSectionDivider,
                _menuSection(
                    menuList: _commonMenu(teams),
                    titleString: "Teams"),
                _menuSectionDivider,
                _menuSection(
                    menuList: _commonMenu(contentAndDisplayRows),
                    titleString: "Content"),
                _menuSectionDivider,
                _menuSection(
                    menuList: _commonMenu(cacheAndCellularRows),
                    titleString: "Cache"),
                _menuSectionDivider,
                _menuSection(
                    menuList: _commonMenu(aboutMenuRows),
                    titleString: "About"),
                _menuSectionDividerSmall,
  ...
  Text appData() {
    return const Text("Octagon 0.0.1/Build 1.0.0",
        style: TextStyle(
            color: Colors.grey,
            fontSize: 12,
  ...
  Widget _commonButton(String btnTitle, Function() toExec,
  ...
  BoxConstraints _menuContainerConstraints(BuildContext ctxt) =>
      BoxConstraints(maxWidth: MediaQuery.of(context).size.width / 1.15);
  ...
  Widget _menuRowWithFunction(
      IconData icon, String rowTitle, Function() toCall) {
    const _style = TextStyle(color: Colors.white);
  ...
  void _handleForgetMeClick(BuildContext context, {String title = "",
  ...
  void _handleLogoutClick(BuildContext context) {
    _handleForgetMeClick.call(context, title: 'Are you sure want to logout!.',
        onYes: (){
