...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
      ...
      switch (event.status) {
        ...
        case Status.COMPLETED:
          ...
          for (int i = 0; i < _messages.length; i++) {
            DateTime createdAt = DateTime.fromMicrosecondsSinceEpoch(_messages[i].createdAt!);
              ...
              body: BlocConsumer(
                  ...
                  listener: (context, state) {
                    ...
                    if (state is UploadFileState) {
                      _handleSendPressed(
                          file: state.fileUploadResponseModel.success!,
                          message: postText);
                    }
                  },
                  builder: (context, _) {
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            //         children: [
                            //           Text(
                            //             "Date:  1 indian india 2 javajavjava ",//${getLiveScoreDate("${snapshot.data?.startDate??''}")}
                            //             style: TextStyle(color: Colors.green, fontSize: 11), maxLines: 2,
                            //           ),
                            //         ],
                            //       ),
                            //     // if(snapshot.data?.startDate!=null && getScoreData(snapshot.data!).isNotEmpty)
                            ...
                            //           Flexible(
                            //             child: Text(
                            //               "Score: 1 indian india 2 javajavjava 1 indian india 2 javajavjava ",//${getScoreData(snapshot.data!)}
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        DateUtil.dateWithDayFormat(DateTime.fromMicrosecondsSinceEpoch(message.createdAt!)),
                        style: const TextStyle(
                      ...
                      _displayTextInputDialog(
                        ...
                        if (messageController.text.trim().isNotEmpty &&
                          ...
                          _handleSendPressed(
                              ...
                              firebaseToken:
                                  message.metadata?["firebaseToken"],
      ...
      try {
        ...
        if (imageSource == null) {
          ...
          if (pickedFileList?.path != null) {
            isImage = false;
            imageItems.add(
                PostFile(filePath: pickedFileList!.path, isVideo: !isImage));
          }
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (BuildContext context) =>
                      VideoEditor(file: File(imageItems[0].filePath)),
                ),
              ).then((value) {
                if (value != null) {
                  // String thumbUrl = "";
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => ImageEditor(
                    ...
                    // allowCamera: false,
                    // allowGallery: false,
                    savePath: Directory.fromUri(
                            Uri.file(imageItems[0].filePath.toString()))
                        .toString(),
  ...
  void _handleSendPressed(
    ...
    if (oldMessage != null) {
      ...
      ///user image
      chatMessage.senderTeam = user?.lastName ?? "${storage.read("userDefaultTeam")}" ?? "";
      if(user!=null){
  ...
  void openSelectedFile(List<String> list, bool isImage) {
      ...
      children: [
        ...
        Row(
          ...
          children: [
            GestureDetector(
              onTap: () {
                PostResponseModelData value=  PostResponseModelData(/*id: data.id,*/
                  title: data.name, comments: [],
                  userId: int.tryParse(data.author.id)??0, userName:  data.author.firstName,
                  type: "1",
                  images: [],
                  photo: data.author.imageUrl,
                  sportInfo: [SportInfo(strSportThumb: data.author.imageUrl,
                      team: [TeamResponseModel(strTeamLogo: data.author.lastName)])],
                  videos: [ImageData(filePath: data.uri??"")],
  ...
  void updateLiveData() {
    setState(() {
      Matche? value = scoreData.firstWhereOrNull((element) => element.matchVs!.toLowerCase().contains((widget.sportInfo?.strTeam??'').toLowerCase()));
      if(value!=null){
  ...
  String getScoreData(Matche currentMatch) {
    String score = "";
      if (currentMatch.matchTeams != null && currentMatch.matchTeams!.isNotEmpty) {
        MatchTeam? firstTeam = currentMatch.matchTeams?.first;

        if(firstTeam!=null){///first team name.
          score = firstTeam.teamName?? "";

          if (firstTeam.matchScore != null && firstTeam.matchScore!.isNotEmpty) {
            MatchScore? lastScore = firstTeam.matchScore?.last;///latest score of first team.
            ///first team score.
            if (lastScore != null) {
        ...
        if(secondTeam!=null){///second team name.
          score = "$score ${secondTeam.teamName?? ""}";

          if (secondTeam.matchScore != null && secondTeam.matchScore!.isNotEmpty) {
            MatchScore? lastScore = secondTeam.matchScore?.last;///latest score of second team.
            ///second team score.
            if (lastScore != null) {
              score = "$score ${lastScore.score??"0"}";
            }else{
