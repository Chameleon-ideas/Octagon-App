  ...
  ChatMessageData(
      {
        this.firebaseToken,
        this.docId,
        this.senderUid, this.senderName, this.image = "", this.timeAgo = "", this.video = "", this.isRead = false,
        this.createdOn,
        this.content,
        this.senderImage,
        this.senderTeam,
  ...
  factory ChatMessageData.fromJson(Map<String, dynamic> json) {
    return ChatMessageData(
      ...
      content: json["content"],
        ...
        likeUsers: json["likeUsers"]==null || json["likeUsers"].toString().length < 4? [] :
        List<String>.from(jsonDecode(json["likeUsers"]).map((x) => x)),
      // updatedAt: DateTime.parse(json["updatedAt"]),
        replay: json["replay"]==null || json["replay"].toString().length < 4? [] :
        List<ChatReplayMessage>.from(jsonDecode(json["replay"]).map((x) => ChatReplayMessage.fromJson(x))),
    );
  }
